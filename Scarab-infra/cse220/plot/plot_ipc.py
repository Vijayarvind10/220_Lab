...
def read_stat_value(stat_path, key):
    try:
  ...
  try:
    for config_key in descriptor_data["configurations"].keys():
      ...
      for benchmark in benchmarks_org:
        ...
        IPC = 0
        mem_stat = exp_path + config_key + '/memory.stat.0.csv'
        # Newer Scarab dumps Periodic_Cycles/Periodic_Instructions instead of Periodic IPC.
        periodic_cycles = read_stat_value(mem_stat, 'Periodic_Cycles')
        periodic_insts = read_stat_value(mem_stat, 'Periodic_Instructions')
        if periodic_cycles and periodic_insts:
          IPC = periodic_insts / periodic_cycles
        else:
          ipc_from_file = read_stat_value(mem_stat, 'Periodic IPC')
          IPC = ipc_from_file if ipc_from_file is not None else 0
